...
| `[x]` / `[ ]` checkbox | `.clinpub/ROADMAP.md` | 追踪 Plan 完成状态 |
...
grep -E "^\*\*Phase $PHASE" .clinpub/ROADMAP.md
```
...
- Phase 0: project_config.yml 关键字段非空
- Phase 1: cleaned.csv 存在
- Phase 2: 当前 Wave 的 SUMMARY.md 存在（检查 .clinpub/phases/02-断点续做/）
- Phase 3: manuscript.md 存在
- Phase 4: final/manuscript.md 存在
...
> Skipped — `workflow.nyquist_validation` is explicitly `false` in `.clinpub/config.json`.
...
| V4 Access Control | Yes | Phase 2 的新命令本身不绕过 workflow-guard.js 的 hook 控制。STATE.md/ROADMAP.md 在 `.clinpub/` 目录下，属于 always-allowed（workflow-guard.js:47-59），所有 Phase 均可写入 |
...
- [VERIFIED: .clinpub/ROADMAP.md] — Plan `[x]`/`[ ]` 格式，Phase 状态 emoji 格式
- [VERIFIED: .clinpub/STATE.md] — 当前 Phase 标识行，`## 下一步` 节格式
