...
在阶段 2 完成后启用对分析输出的定向修改或新方法添加。通过结构化的 定义->执行->验证->记录 周期编排修改流程。可在阶段 2、3、4 中调用，当用户需要调整分析结果或新增分析方法时使用。
...
2. **修改**：执行变更（R 脚本修改 + 重跑）
   - 新增方法时，方法 ID 必须遵循 `{NN}_{MethodName}` 格式，禁止裸下划线前缀
   - 必须同时创建 `03_AnalysisMethods/{id}/` 和 `04_Outputs/{id}/`
3. **简报**：报告每项成功/失败
