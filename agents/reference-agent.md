...
description: "Literature search and reference management specialist. Uses ncbi-search skill (PubMed) for all literature searches. Outputs citation_map.md and references.bib in Vancouver format with DOIs."
...
Optional: check `NCBI_API_KEY` env var (improves rate limit from 3req/s to 10req/s). If not set, inform user but proceed — ncbi-search works without API key.
</step>
...
使用 `skill("ncbi-search")` 加载 ncbi-search 技能，然后按其指引执行 PubMed 搜索：

```
1. 加载技能: skill("ncbi-search")
2. 按技能指引搜索 PubMed 数据库
3. 搜索参数:
   - 查询: {从 project_config.yml 和讨论中提取的关键词}
   - 年份过滤: 默认近 {year_range.max_years_ago} 年
...
- Supplementary search per chapter topic via ncbi-search skill
- Full-text retrieval via DOI → Unpaywall → pdf-reader

**Phase 4 (review):**
- Targeted supplementary search for reviewer-raised topics via ncbi-search skill
</step>
...
**搜索策略（使用 ncbi-search 技能）：**
调用 `skill("ncbi-search")`，在 PubMed 中搜索方法学文献：

```
搜索内容: "{method_name} statistical method"
参数:
  - 数据库: pubmed
  - 类型优先: review（综述）
  - 如罕见方法: 搜索 clinical_trial（应用案例）
  - 最大返回: 5 篇
```
...
2. 使用 `skill("ncbi-search")` 搜索 PubMed：
   - 搜索: "{phase_keywords} clinical research"
   - 类型优先: review
   - 最大返回: 5 篇
...
2. 使用 `skill("ncbi-search")` 在 PubMed 中搜索技术文献：
   - 搜索: "{technology_keyword} implementation methods"
   - 最大返回: 5 篇
...
**搜索注意事项：**
- 所有搜索统一使用 ncbi-search 技能（per D-09）
...
### method_search 规则
- 方法搜索统一使用 ncbi-search 技能（PubMed）
...
- Track A 使用 ncbi-search 技能搜索 PubMed
- Track B 先读 codebase 地图再用 ncbi-search 搜索技术文献
