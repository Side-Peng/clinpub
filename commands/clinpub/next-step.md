...
Phase 行读取:
```bash
grep -oP '阶段：Phase\s*\K\d' .planning/STATE.md
```

Plan checkbox 读取（ROADMAP.md）:
```bash
# 读取指定 Phase 的 Plan 完成状态
completed=$(grep -c "\[x\] $PHASE-" .planning/ROADMAP.md)
total=$(grep -c "\- \[.\] $PHASE-" .planning/ROADMAP.md)
```
...
```
MILESTONE_PATH=".planning/phases/{N}-{phase-slug}/MILESTONE.md"
...
**提示**: 以上信息已同步更新到 `.planning/STATE.md` 的「下一步」节。
```
