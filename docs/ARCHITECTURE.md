...
│   ├── review.md              # 审稿修稿
│   └── milestone.md           # 关卡评审
│   （init-project 已移至 ~/.claude/commands/clinpub-init.md）
...
用户通过 Claude Code 的 Skill 系统调用命令。每个命令文件定义了触发条件、参数和执行流程。

**入口点**：`commands/clinpub/clinpub.md` 是主入口，根据用户输入路由到具体命令。
